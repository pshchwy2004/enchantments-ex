...
public record ShulkerRedirectorEffect(LevelBasedValue amount) implements EnchantmentEntityEffect {
    ...
    @Override
    public void apply(ServerLevel world, int level, @NonNull EnchantedItemInUse context, Entity target, @NonNull Vec3 pos) {
        // per tick
        ...
        for (ShulkerBullet bullet : bullets) {
            if (bullet.getOwner() != null && bullet.getOwner() instanceof Shulker) {
                ...
                if (((ShulkerBulletAccessor) bullet).enex$getFinalTarget().matches(bullet.getOwner()) && ((ProjectileAccessor) bullet).enex$getLeftOwner()) {
                    ((ShulkerBulletAccessor) bullet).enex$setFinalTarget(EntityReference.of(bullet.getOwner()));
                }
