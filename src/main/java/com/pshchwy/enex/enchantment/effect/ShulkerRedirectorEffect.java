...
public record ShulkerRedirectorEffect(LevelBasedValue amount) implements EnchantmentEntityEffect {
    ...
    public void apply(ServerLevel world, int level, EnchantedItemInUse context, Entity target, Vec3 pos) {
        ...
        for (ShulkerBullet bullet : bullets) {
            if (bullet.getOwner() != null && bullet.getOwner() instanceof Shulker && ((ShulkerBulletAccessor) bullet).enex$getFinalTarget() != bullet.getOwner() && ((ProjectileAccessor) bullet).enex$checkLeftOwner()) {
